...
public class StackSort {
    ...
    private static int[] doStackSort(int[] data) {
        ...
        int[] result = new int[data.length]; // sorted in ascending order
        VectorStack<Integer> lowerValues = new VectorStack<>();
        VectorStack<Integer> upperValues = new VectorStack<>();

        // push all elements from the data array into upperValues
        ...
        while (!upperValues.isEmpty()) {
            ...
            // move elements from lowerValues to upperValues if necessary to maintain order
            while (!lowerValues.isEmpty() && lowerValues.peek() > x) {
                upperValues.push(lowerValues.pop());
            }

            // push the current element into lowerValues
            lowerValues.push(x);
        }

        // move all values from lowerValues to upperValues
        while (!lowerValues.isEmpty()) {
            upperValues.push(lowerValues.pop());
        }

        // move sorted values into result
        int index = 0;
        while (!upperValues.isEmpty()) {
